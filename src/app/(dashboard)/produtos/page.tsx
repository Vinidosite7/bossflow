    ...
    <div className="flex flex-col gap-6">
      ...
      <motion.div {...fadeUp(0)} className="flex items-center justify-between" data-tour="produtos-header">
        ...
        <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
          ...
          <Plus size={15} /> <span className="hidden sm:inline">Novo produto</span><span className="sm:hidden">Novo</span>
        </motion.button>
      </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }}
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => { setShowForm(false); setEditProduct(null) }} style={{ color: '#4a4a6a' }}><X size={18} /></button>
              </div>
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                <input type="text" placeholder="Nome do produto *" value={form.name} required
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium" style={{ color: '#6b6b8a' }}>Preço *</label>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium" style={{ color: '#6b6b8a' }}>Unidade</label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium" style={{ color: '#6b6b8a' }}>Estoque (opcional)</label>
                ...
                <button type="submit" disabled={saving}
                  ...
                  {saving ? <Loader2 size={16} className="animate-spin" /> : editProduct ? 'Salvar alterações' : 'Criar produto'}
                </button>
              </form>
            </motion.div>
          </motion.div>
      ...
      {filtered.length === 0 ? (
        <motion.div {...fadeUp(0.1)} className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p style={{ color: '#4a4a6a' }}>{search ? 'Tente outro termo' : 'Cadastre seu primeiro produto'}</p>
        </motion.div>
      ) : (
            ...
            {filtered.map((product, i) => (
              <motion.div key={product.id}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate" style={{ color: '#e8e8f0' }}>{product.name}</p>
                ...
                <div className="flex gap-2 shrink-0">
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                    onClick={() => openEdit(product)} className="w-7 h-7 rounded-lg flex items-center justify-center"
                    style={{ background: 'rgba(124,110,247,0.1)', color: '#7c6ef7' }}>
                    <Pencil size={12} />
                  </motion.button>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                    onClick={() => setShowConfirm(product.id)} className="w-7 h-7 rounded-lg flex items-center justify-center"
                    style={{ background: 'rgba(248,113,113,0.1)', color: '#f87171' }}>
                    <Trash2 size={12} />
                  </motion.button>
                </div>
