...
import {
  Target, Zap, Trophy, CheckCircle2,
  TrendingUp, X, Save,
} from 'lucide-react'
...
export default function MetasPage() {
  ...
  const {
    goals, loading, saveGoal,
    annualTarget, annualRevenue, annualPct,
    monthsHit, streak,
  } = useGoals(businessId)
  ...
  async function handleSave() {
    ...
    await saveGoal(
      ...
      editSuper ? parseFloat(editSuper) : null,
    )
    setSaving(false)
  ...
  const badges = [
    {
      id: 'first_goal', emoji: '🚀', label: 'Primeiro passo',
      desc: 'Definiu sua primeira meta',
      unlocked: goals.some(g => g.target > 0), color: '#7c6ef7',
    },
    {
      id: 'first_hit', emoji: '🎯', label: 'Em cheio!',
      desc: 'Bateu a meta pela primeira vez',
      unlocked: goals.some(g => g.hit), color: '#34d399',
    },
    {
      id: 'super_hit', emoji: '⚡', label: 'Super Cota',
      desc: 'Atingiu a super cota',
      unlocked: goals.some(g => g.superHit), color: '#a78bfa',
    },
    {
      id: 'streak_3', emoji: '🔥', label: '3 meses seguidos',
      desc: 'Bateu a meta 3 meses consecutivos',
      unlocked: streak >= 3, color: '#f97316',
    },
    {
      id: 'streak_6', emoji: '🌟', label: '6 meses seguidos',
      desc: 'Bateu a meta 6 meses consecutivos',
      unlocked: streak >= 6, color: '#fbbf24',
    },
    {
      id: 'annual_50', emoji: '📈', label: 'Metade do caminho',
      desc: '50% da meta anual atingida',
      unlocked: annualPct >= 50, color: '#22d3ee',
    },
    {
      id: 'annual_100', emoji: '🏆', label: 'Ano batido!',
      desc: 'Meta anual 100% atingida',
      unlocked: annualPct >= 100, color: '#fbbf24',
    },
    {
      id: 'perfect_year', emoji: '💎', label: 'Ano perfeito',
      desc: 'Bateu todas as metas do ano',
      unlocked: monthsHit === 12, color: '#a78bfa',
    },
  ]
  ...
  // Só bloqueia enquanto busca o businessId — não bloqueia enquanto carrega as metas
          ...
          <>
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-3xl font-bold" style={{ fontFamily: 'Syne, sans-serif', color: annualColor }}>
              ...
              <div className="relative w-16 h-16">
                <svg viewBox="0 0 64 64" className="w-16 h-16 -rotate-90">
                  ...
                  <motion.circle cx="32" cy="32" r="26" fill="none" stroke={annualColor} strokeWidth="5"
                    strokeLinecap="round"
            ...
            <div className="h-2 rounded-full overflow-hidden" style={{ background: '#1e1e2e' }}>
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(annualPct, 100)}%` }}
                transition={{ duration: 1.2, ease: 'easeOut' }}
              ...
              return (
                <motion.button
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold" style={{ color: isCurrent ? '#f97316' : '#6b6b8a' }}>
                  ...
                  {hasGoal ? (
                    <>
                      <p className="text-sm font-bold" style={{ color: isFuture ? '#4a4a6a' : color }}>
                        {Math.round(g.pct)}%
                      </p>
                    ...
                    <p className="text-xs mt-1" style={{ color: '#3a3a5c' }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {badges.map((b, i) => (
            <div key={b.id}
              ...
              <div>
                <p className="text-xs font-bold leading-tight" style={{ color: b.unlocked ? b.color : '#4a4a6a' }}>
                  {b.label}
                </p>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-1.5" style={{ color: '#4a4a6a' }}>
                    Meta bônus extra ambiciosa — desbloqueie o badge ⚡ ao atingir
                  </p>
                </div>
